...
int wString::load_from_file(const char* FileName)
{
	...
	if (strncmp(FileName, "http://", 7) == 0) {
		wString tmp;
		//tmp = wString::http_get((char*)FileName,0);
		*this = tmp;
	}
	else {
#ifdef linux
...
//---------------------------------------------------------------------------
wString wString::http_get(const char* url, off_t offset)
{
	int         recv_len;                       //読み取り長さ
	wString     buf;
	wString     ptr;
...
// Linux用ファイル名に変換
//---------------------------------------------------------------------------
char* wString::get_local_address(void)
{
#ifdef linux
